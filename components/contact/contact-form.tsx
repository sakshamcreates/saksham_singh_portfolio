...
export function ContactForm() {
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    if (!serviceId || !templateId || !publicKey) {
      ...
      setSubmitError(
        "Email service is not configured. Please try again later or reach out directly via email."
      );
      return;
    }
              ...
              <AnimatePresence>
                {submitError && (
                  <motion.div
                    ...
                    animate={{ opacity: 1, y: 0 }}
                    exit={{ opacity: 0, y: -8 }}
                    className="flex items-center gap-2 rounded-md bg-red-100 px-4 py-3 text-sm text-red-600 dark:bg-red-900/30 dark:text-red-300"
                  >
